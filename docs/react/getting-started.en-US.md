...
import { DatePicker, message } from 'antd-hz';
...
```diff
- import { DatePicker, message } from 'antd-hz';
+ import { DatePicker, message, Alert } from 'antd-hz';
```
...
`antd` supports tree shaking of ES modules, so using `import { Button } from 'antd-hz';` would drop js code you didn't use.
